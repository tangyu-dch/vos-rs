  ...
  it('removes mutually exclusive policy fields before saving', () => {
    expect(
      policyForSave({
        ...
        fixed_number: '10086',
        caller_pool_id: 'pool-a',
        egress_mode: 'direct',
        direct_egress_trunk_id: 'carrier-a',
        egress_group_id: 'group-a',
      }),
    ).toEqual({
      caller_mode: 'strict_passthrough',
      fallback_mode: 'reject',
  ...
  it('requires a new password when the digest username changes', () => {
    expect(
      trunkValidationError(
        {
          role: 'access',
          access_auth_mode: 'digest_register',
          access_username: 'new-user',
          has_access_password: true,
        },
  ...
  it('requires an enabled IP row and validates entered egress endpoints', () => {
    ...
    expect(
      trunkValidationError(
        ...
        {},
        [],
        [{ host: '', transport: 'udp', enabled: false }],
      ),
    ).toBe('落地端点的主机地址不能为空');
  });
  ...
  it('validates egress trunk registration when connection type is client_register', () => {
    ...
    expect(
      trunkValidationError(
        {
          role: 'egress',
          egress_connection_type: 'client_register',
          register_server: 'sip.carrier.com',
        },
        {},
        [],
        [],
