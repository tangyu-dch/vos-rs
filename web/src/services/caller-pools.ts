...
export function callerPoolValidationError(
  pool: CallerPool,
  ...
  for (const member of members) {
    ...
    if (
      ...
      return `真实号码 ${number} 的优先级必须是 0 到 65535 的整数`;
    if (
      !Number.isInteger(member.weight) ||
      Number(member.weight) < 1 ||
      Number(member.weight) > 10000
    )
      return `真实号码 ${number} 的权重必须是 1 到 10000 的整数`;
    if (!Number.isInteger(member.max_concurrent) || Number(member.max_concurrent) < 0)
      return `真实号码 ${number} 的最大并发必须是非负整数`;
  }
  return null;
}
...
export async function updateCallerPool(id: string, pool: CallerPool): Promise<void> {
  return api.put<void>(`/caller-pools/${encodeURIComponent(id)}`, {
    ...pool,
...
export async function saveCallerPoolMembers(
  id: string,
  members: CallerPoolMember[],
): Promise<void> {
