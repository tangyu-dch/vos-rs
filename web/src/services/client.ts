...
export function shouldRetryRequest(
  ...
  const isTransient =
    ...
    status === 502 ||
    (!hasResponse && errorCode !== 'ECONNABORTED');
...
http.interceptors.response.use(undefined, async (error) => {
  ...
  const message =
    body?.details ||
    body?.message ||
    (isTimeout ? '请求超时，请检查服务状态' : '请求失败，请稍后重试');
...
export function unwrap<T>(payload: Envelope<T> | T): T {
  if (payload && typeof payload === 'object' && 'data' in payload && 'code' in payload) {
    ...
    if (envelope.code !== 0)
      throw new ApiError(envelope.message, undefined, String(envelope.code), envelope.request_id);
