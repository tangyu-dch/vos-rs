...
export async function getTrunkWorkspace(id: string): Promise<TrunkWorkspaceData> {
  ...
  const trunk =
    result.trunk && typeof result.trunk === 'object' ? (result.trunk as Entity) : result;
  return {
    trunk,
...
export function trunkValidationError(
  ...
  if (role === 'access') {
    ...
    if (!['ip_allowlist', 'digest_register', 'ip_and_digest'].includes(mode))
      return '请选择接入认证方式';
    if (rules.some((rule) => !rule.cidr.trim())) return 'IP 白名单地址不能为空';
    if (needsIp && !rules.some((rule) => rule.enabled))
      return '请至少配置一条已启用且完整的 IP 白名单';
    if (needsDigest) {
  ...
  if (role === 'egress') {
    ...
    if (connectionType === 'client_register') {
      ...
      const usernameChanged = original.reg_username
        ? username !== String(original.reg_username).trim()
        : false;
...
export function saveOutboundPolicy(id: string, policy: OutboundPolicy) {
  return api.put<OutboundPolicy>(
    `/trunks/${encodeURIComponent(id)}/outbound-policy`,
...
export function policyValidationError(policy: OutboundPolicy): string | null {
  if (policy.caller_mode === 'fixed_number' && !policy.fixed_number?.trim())
    return '固定号码策略必须选择真实号码';
  if (policy.caller_mode === 'virtual_pool' && !policy.caller_pool_id?.trim())
    return '虚拟主叫策略必须选择号码池';
  if (policy.egress_mode === 'direct' && !policy.direct_egress_trunk_id?.trim())
    return '请选择直接绑定的落地中继';
  if (policy.egress_mode === 'group' && !policy.egress_group_id?.trim())
    return '请选择允许使用的落地分组';
  return null;
}
