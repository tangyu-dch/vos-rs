...
function HourlyTrendsSection({ trends }: { trends?: HourlyTrendItem[] }) {
  ...
  const pointsAnswered = displayData
    .map((d, idx) => {
      const x = (idx / (displayData.length - 1)) * chartWidth;
  ...
  const handleMouseMove = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    const idx = Math.min(
      Math.max(Math.round(pct * (displayData.length - 1)), 0),
  ...
  return (
    <Card
      shadow="none"
      className="overview-card w-full dash-enter"
      ...
      <CardBody className="p-5 flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-small font-semibold text-foreground">话务趋势</h3>
          ...
          <div className="flex items-center gap-3 text-tiny">
            ...
            <span className="text-default-300">|</span>
            <span className="flex items-center gap-1.5">
              <span className="text-default-500">接通率</span>
              <span className="font-semibold text-foreground tnum">{overallAsr.toFixed(1)}%</span>
            </span>
          </div>
        </div>
        ...
        <div className="relative" ref={chartRef}>
          ...
          {hoveredIdx !== null && hoveredData && (
            <div
              className="absolute z-50 pointer-events-none bg-background/95 shadow-sm border border-default-200 p-2.5 rounded-lg text-tiny flex flex-col gap-1 w-44 dash-enter"
              style={{
...
function MosGauge({ score }: { score: number }) {
  ...
  const colorClass =
    clampedScore >= 4.0 ? 'text-success' : clampedScore >= 3.5 ? 'text-warning' : 'text-danger';
  const r = 50;
  ...
  return (
    <svg
      ...
      role="img"
      aria-label={`MOS 语音质量评分 ${clampedScore.toFixed(2)}`}
    >
      <path
      ...
      <line
        ...
        strokeWidth={1.5}
      />
      <text x={tick3_5.x - 2} y={tick3_5.y + 12} className="fill-warning text-[8px] font-mono">
        3.5
      </text>
      <text x={tick4_0.x - 2} y={tick4_0.y + 12} className="fill-success text-[8px] font-mono">
        4.0
      </text>
      <text
        x={60}
        y={50}
        textAnchor="middle"
        className="fill-foreground text-lg font-bold font-mono tnum"
      >
...
function MosQualitySection({ data }: { data: Summary }) {
  ...
  return (
    <Card
      shadow="none"
      className="overview-card dash-enter h-full"
      ...
      <CardBody className="p-5 flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-small font-semibold text-foreground">媒体质量</h3>
          </div>
          <Chip size="sm" variant="flat" color={mosColor}>
            {mosLabel}
          </Chip>
        </div>
        ...
        <div className="p-4 bg-content2 rounded-xl flex items-center justify-between gap-3">
          <div className="flex flex-col gap-1">
            <div className="text-tiny text-default-500 font-medium">平均质量分</div>
            <div className="text-2xl font-semibold text-foreground tnum">{mosScore.toFixed(2)}</div>
            <div className="text-[10px] text-default-400">满分 5.00</div>
          </div>
      ...
      <CardBody className="p-5 flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-small font-semibold text-foreground">信令质量</h3>
          </div>
        ...
        <div className="flex flex-col gap-3">
          <div className="flex flex-col gap-1.5 p-3 rounded-medium bg-content2">
            <div className="flex justify-between text-tiny">
              <span className="font-medium text-default-500">接通率</span>
              <span className="font-semibold text-foreground tnum">{asr.toFixed(3)}%</span>
            </div>
            <Progress
              size="sm"
              value={asr}
          ...
          <div className="flex flex-col gap-1.5 p-3 rounded-medium bg-content2">
            <div className="flex justify-between text-tiny">
              <span className="font-medium text-default-500">有效到达率</span>
              <span className="font-semibold text-foreground tnum">{ner.toFixed(3)}%</span>
            </div>
            <Progress size="sm" value={ner} color="primary" aria-label="网络有效到达率" />
          </div>

          <div className="grid grid-cols-3 gap-2 text-center">
            <div className="p-2 rounded-medium bg-content2/60">
              <div className="text-[10px] text-default-400">已接通</div>
              <div className="text-sm font-semibold text-foreground tnum">
                {data.today_answered_calls ?? 0}
              </div>
            </div>
            <div className="p-2 rounded-medium bg-content2/60">
              <div className="text-[10px] text-default-400">已取消</div>
              <div className="text-sm font-semibold text-foreground tnum">
                {data.today_canceled_calls ?? 0}
              </div>
            </div>
            <div className="p-2 rounded-medium bg-content2/60">
              <div className="text-[10px] text-default-400">失败</div>
              <div className="text-sm font-semibold text-foreground tnum">
                {data.today_failed_calls ?? 0}
              </div>
            </div>
          </div>
        </div>
...
function NodeTrafficSection() {
  ...
  const filteredData = trafficData.filter(
    (d) => selectedType === 'all' || d.node_type === selectedType,
  );
  ...
  const handleMouseLeave = () => setHoveredIdx(null);
  const hoveredX =
    hoveredIdx !== null && allHours.length > 1
      ? (hoveredIdx / (allHours.length - 1)) * chartWidth
      : 0;
      ...
      <CardBody className="p-5 flex flex-col gap-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-small font-semibold text-foreground">节点流量</h3>
            <span className="text-default-300">·</span>
          ...
          <div className="flex flex-col gap-3">
              ...
              {filteredData.map((node, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="font-mono font-semibold text-foreground text-tiny">
                      {node.node_id}
                    </span>
                    <span className="text-default-400 text-[10px] font-mono">
                      {node.node_type === 'sip' ? 'SIP' : 'RTP'}
                    </span>
                    <span className={`font-mono font-bold text-tiny tnum ${c.text}`}>
                      {formatKbps(latestKbps)}/s
                    </span>
                  </div>
                );
              })}
            ...
            <div className="relative" ref={chartRef}>
              ...
              >
                ...
                {filteredData
                  .filter((d) => !hiddenNodes.has(d.node_id))
                  .map((node, nodeIdx) => {
                        ...
                        {node.series.map((item, idx) => {
                          ...
                          return (
                            <circle
                              ...
                              cy={y}
                              r="3"
                              className={`${c.fill} stroke-background`}
                              strokeWidth="1.5"
                            />
                          );
                        })}
              ...
              {hoveredIdx !== null && hoveredTime && (
                <div
                  className="absolute z-50 pointer-events-none bg-background/95 shadow-sm border border-default-200 p-2.5 rounded-lg text-tiny flex flex-col gap-1 w-52 dash-enter"
                  style={{
...
function MonitoringExtrasSection() {
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {Array.from({ length: 3 }).map((_, idx) => (
          <Card key={idx} shadow="none" className="overview-card w-full h-48 animate-pulse">
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {/* 中继并发水位 */}
      <Card shadow="none" className="overview-card dash-enter" style={{ animationDelay: '440ms' }}>
        <CardBody className="p-5 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Server className="w-4 h-4 text-primary" />
              <h3 className="text-small font-semibold text-foreground">中继水位</h3>
            </div>
            ...
            {[...gateways]
              ...
              .map((gw) => {
                ...
                const percent = hasLimit
                  ? Math.round((gw.active_calls / gw.max_channels) * 100)
                  : 0;
                return (
                  <div key={gw.name} className="p-2.5 bg-content2 rounded-lg flex flex-col gap-1.5">
                    <div className="flex justify-between items-center text-tiny">
                      <div className="flex items-center gap-1.5">
                        <span
                          className={`w-1.5 h-1.5 rounded-full ${gw.direction === 'access' ? 'bg-primary' : 'bg-success'}`}
                        />
                        <span className="font-mono font-bold text-foreground">{gw.name}</span>
                        <span className="text-default-400 text-[9px] font-mono">
                          {gw.direction === 'access' ? '接入' : '落地'}
                        </span>
                      </div>
                      <span className="font-mono font-semibold text-foreground tnum">
                        {gw.active_calls}/{hasLimit ? gw.max_channels : '∞'}
                      </span>
                    </div>
                    {hasLimit ? (
                      <Progress
      ...
      {/* SBC 安全防御 */}
      <Card shadow="none" className="overview-card dash-enter" style={{ animationDelay: '480ms' }}>
        <CardBody className="p-5 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Shield className="w-4 h-4 text-warning" />
              <h3 className="text-small font-semibold text-foreground">安全防御</h3>
            </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="p-3 rounded-lg bg-default-50 flex flex-col gap-1">
              <span className="text-[10px] text-default-500 font-medium">拦截次数</span>
              <span className="text-xl font-semibold text-foreground tnum">
                {security.blocked_calls_24h}
              </span>
            </div>
            <div className="p-3 rounded-lg bg-default-50 flex flex-col gap-1">
              <span className="text-[10px] text-default-500 font-medium">鉴权失败</span>
              <span className="text-xl font-semibold text-foreground tnum">
          ...
          <div className="flex flex-col gap-1.5">
            <span className="text-[10px] text-default-400 font-medium">失败响应码分布</span>
            <div className="flex flex-wrap gap-1.5">
              {Object.entries(security.error_codes_breakdown).map(([code, count]) => {
                ...
                return (
                  <div
                    key={code}
                    className="flex items-center gap-1 bg-content2 px-2 py-1 rounded-medium text-tiny font-mono"
                  >
                    <Chip size="sm" color={color} variant="flat" className="h-4 px-1 text-[10px]">
                      {code}
                    </Chip>
      ...
      {/* 系统资源 */}
      <Card shadow="none" className="overview-card dash-enter" style={{ animationDelay: '520ms' }}>
        <CardBody className="p-5 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Cpu className="w-4 h-4 text-success" />
              <h3 className="text-small font-semibold text-foreground">资源状态</h3>
            </div>
          ...
          <div className="flex flex-col gap-3">
            <div className="flex flex-col gap-1">
              <div className="flex justify-between text-tiny items-center">
                ...
                <span className="font-mono font-bold text-foreground tnum">
              ...
              <Progress
                ...
                color={resources.cpu_percent >= 80 ? 'danger' : 'success'}
                aria-label="处理器"
              />
            </div>
            <div className="flex flex-col gap-1">
              ...
              <Progress
                ...
                color={resources.memory_percent >= 85 ? 'danger' : 'success'}
                aria-label="内存"
              />
            </div>
            <div className="flex flex-col gap-1">
              <div className="flex justify-between text-tiny items-center">
                ...
                <span className="font-mono font-bold text-foreground tnum">
                  {(100 - resources.disk_percent).toFixed(1)}%
                </span>
              </div>
              <Progress
                size="sm"
                value={resources.disk_percent}
                color={resources.disk_percent >= 90 ? 'danger' : 'primary'}
                aria-label="存储"
              />
            </div>
            ...
            <div className="text-tiny text-default-400 border-t border-divider pt-2 mt-1 flex items-center justify-between">
              ...
              <span className="text-foreground font-semibold tnum">
                {resources.db_pool_active}/{resources.db_pool_max} 已用
              </span>
            </div>
          </div>
        </CardBody>
...
function PrimaryKpiCard({
  ...
  return (
    <Card
      shadow="none"
      className="overview-card dash-enter overflow-hidden relative"
    ...
    >
      <CardBody className="p-4 flex flex-col gap-2.5">
        <div className="flex items-center justify-between">
          ...
          <div
            className={`w-8 h-8 rounded-lg bg-default-100 flex items-center justify-center ${color}`}
          >
            {icon}
          </div>
        </div>
...
function SecondaryKpiCard({
  ...
  return (
    <Card
      shadow="none"
      className="overview-card dash-enter"
      style={{ animationDelay: `${delay}ms` }}
    >
      <CardBody className="p-4 flex flex-col gap-1.5">
        ...
        <div className="flex items-baseline gap-1">
          <span className="text-xl font-semibold tracking-tight text-foreground tnum">{value}</span>
          {unit && <span className="text-[10px] text-default-400">{unit}</span>}
        </div>
      </CardBody>
    </Card>
  );
...
function RuntimeStatusCard() {
  return (
    ...
    >
      <CardBody className="p-4 flex flex-col gap-4 justify-between">
        <div>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <Radio className="w-4 h-4 text-success" />
              <h3 className="text-small font-semibold text-foreground">运行引擎</h3>
            </div>
            <Chip size="sm" variant="dot" color="success">
              运行正常
            </Chip>
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between text-tiny p-3 rounded-medium bg-default-50 font-medium">
        ...
        <div className="text-tiny text-default-400 border-t border-divider pt-3 flex items-center justify-between">
          ...
          <span className="text-foreground font-semibold tnum">1,700 路</span>
        </div>
      </CardBody>
    </Card>
  );
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 relative z-10">
              <div className="min-w-0">
                <div className="flex items-center gap-2 mb-1.5">
                  <h1 className="text-lg font-semibold text-foreground tracking-tight">运行总览</h1>
                  <Chip
                    color="success"
                    size="sm"
                    variant="flat"
      ...
      {error ? (
        ...
        <Card shadow="none" className="overview-card">
          <CardBody className="py-16 flex justify-center">
            <Spinner color="primary" label="正在加载实时运行指标..." />
          </CardBody>
        </Card>
      ) : (
