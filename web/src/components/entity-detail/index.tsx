...
} from '@/components/sip-flow-diagram';
...
export function DetailFields({ value, empty }: { value: unknown; empty: string }) {
  if (!value || typeof value !== 'object') {
    return <div className="py-8 text-center text-small text-default-400">{empty}</div>;
  }
...
export function EntityDetail({
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      setEntity(await getResource(path, id));
    } catch (e) {
  ...
  const loadTab = async (key: string, subpath?: string, sourceKey?: string) => {
    ...
    try {
      ...
      setRelated((old) => ({
        ...old,
        [key]: { error: e instanceof Error ? e.message : '加载失败' },
      }));
    }
  };
  ...
  const renderObject = (value: unknown, tabKey?: string) => {
    ...
    if (value && typeof value === 'object' && 'error' in value) {
      return (
        <ErrorState
          error={String((value as Entity).error)}
          retry={() => {
            setRelated((old) => {
              const next = { ...old };
              delete next[tabKey!];
    ...
    if (tabKey === 'sipflow' && Array.isArray(value))
      return <SipFlowDiagram events={value as SipFlowEvent[]} />;
    if (tabKey === 'media') {
      const mediaVal = value as CallMediaStatus;
      if (mediaVal.runtime_availability === 'not_active') {
        return (
          <div className="py-8 text-center text-small text-default-400">
            通话已结束，实时媒体流已销毁
          </div>
        );
      }
      if (mediaVal.runtime_availability === 'unavailable') {
        return (
          <Card className="border border-warning-200 bg-warning-50 p-2">
            <CardBody className="p-4 text-small text-warning-700">
              实时媒体引擎不可达，无法获取指标
            </CardBody>
          </Card>
        );
    ...
    return (
      <div className="flex flex-col gap-3">
        {list.map((item, index) => (
          <DetailFields key={index} value={item} empty="暂无数据" />
        ))}
      </div>
    );
  };
  ...
  const root =
    rootKey && entity?.[rootKey] && typeof entity[rootKey] === 'object'
      ? (entity[rootKey] as Entity)
      : entity;
          ...
          <CardBody className="p-4">
            <Tabs
              ...
              onSelectionChange={(key) => {
                ...
                if (tab && related[tab.key] === undefined)
                  void loadTab(tab.key, tab.path, tab.sourceKey);
              }}
            >
              {tabs.map((tab) => (
                <Tab key={tab.key} title={tab.title}>
                  <div className="py-4">
                    {tab.key === 'summary' && path === '/calls' ? (
                      <CallSummary entity={entity} />
                    ) : tab.key === 'summary' && root ? (
                      <DetailFields value={root} empty="暂无详情" />
                    ) : (
