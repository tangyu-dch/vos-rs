...
`media-edge` 是 vos-rs 平台的 **下一代媒体中继服务**。与 `sip-edge` 内置的传统 RTP 中继不同，`media-edge` 采用 eBPF/XDP 内核旁路技术，把 RTP 包处理从用户态下沉到内核态，实现 10 万路+ RTP 转发。
...
| 能力 | 说明 |
| :--- | :--- |
| **eBPF/XDP 内核旁路** | RTP 包在内核态转发，零拷贝，10 万路+ |
