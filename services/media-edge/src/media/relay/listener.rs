...
fn create_reuseport_socket(port: u16) -> io::Result<UdpSocket> {
    let addr = SocketAddr::from(([0, 0, 0, 0], port));
    let std_socket = std::net::UdpSocket::bind(addr)?;
    let _ = crate::media::relay::affinity::enable_socket_reuseport(&std_socket);
...
pub async fn spawn_rtp_relay_listeners(
    ...
    for port in (config.port_min..=config.port_max).step_by(2) {
        let rtp_socket = create_reuseport_socket(port)?;
        sockets.push((rtp_socket, port, MediaPacketKind::Rtp));

        if let Some(rtcp_port) = rtcp_port_for(port) {
            let rtcp_socket = create_reuseport_socket(rtcp_port)?;
...
pub(crate) async fn relay_media_port(
    ...
    let core_id = (local_port as usize / 2) % num_cpus::get();
    let _ = crate::media::relay::affinity::bind_thread_to_cpu_core(core_id);
    ...
    loop {
        ...
        if crate::media::relay::webrtc::is_stun_packet(packet_data) {
            let session = relay
                .webrtc_sessions
                .get(&local_port)
                .map(|entry| entry.clone());
            if let Some(session) = session {
                match session.handle_stun_packet(packet_data, source).await {
                    Ok(response) => {
                        let _ = relay.learn_symmetric_source(local_port, source);
                        if let Err(error) = socket.send_to(&response, source).await {
                            warn!(%error, local_port, %source, "发送 ICE Binding Response 失败");
                        }
                    }
                    Err(error) => {
                        warn!(%error, local_port, %source, "拒绝无效 ICE Binding Request");
                    }
                }
            }
            continue;
        ...
        if crate::media::relay::webrtc::is_dtls_packet(packet_data) {
            if let Some(session) = relay.webrtc_sessions.get(&local_port) {
                if let Err(error) = session.handle_dtls_packet(packet_data) {
                    warn!(%error, local_port, %source, "DTLS 报文分发失败");
                }
            }
            continue;
        }
        ...
        let web_rtc_encrypted = if let Some(session) = web_rtc_target_session {
            match session.encrypt(packet_kind, outbound_packet).await {
                Ok(packet) => Some(packet),
                Err(error) => {
                    relay.record_metric(local_port, |metrics| metrics.send_errors += 1);
                    warn!(%error, local_port, "WebRTC SRTP/SRTCP 加密失败");
                    continue;
                }
            }
        } else {
