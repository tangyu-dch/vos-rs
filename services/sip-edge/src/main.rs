        ...
        if let Some(ref db) = db_store {
            match db.load_gateway_health_list().await {
                Ok(health_list) => {
                    ...
                    for (gw_id, open, failures, _state, last_failure_at, half_open_successes, _last_probe_at) in health_list {
                        let last_failure_sys = last_failure_at.map(|dt| {
                            std::time::UNIX_EPOCH
                                + std::time::Duration::from_secs(dt.unix_timestamp() as u64)
                        });
                        health.restore_state(&gw_id, open, failures, last_failure_sys, half_open_successes);
                    }
