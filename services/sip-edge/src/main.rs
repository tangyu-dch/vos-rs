...
pub(crate) use timers::{
    spawn_client_transaction_retransmission, spawn_session_timer_watchdog,
    spawn_nat_keepalive_loop, calculate_mos_for_legs,
};
    ...
    async fn is_peer_gateway(&self, peer: SocketAddr) -> bool {
        ...
        let result = if let Some(ref db) = self.db_store {
            if let Ok(gateways) = db.load_gateways().await {
                gateways.iter().any(|(_, host, _, _, _, _, _, _)| host == &peer_ip)
            } else {
                false
            }
        } else {
