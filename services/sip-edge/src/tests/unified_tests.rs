...
fn test_scenario_runner() {
    ...
    println!(
        "Running parameter-based unified test scenario: {}",
        scenario
    );
...
async fn register_contact(edge_state: &EdgeState, user: &str, host: &str, port: u16) {
    ...
    let datagrams = handle_datagram(register.as_bytes(), peer, edge_state, &edge_config()).await;
...
async fn register_stores_contact_and_returns_binding() {
    ...
    let datagrams = handle_datagram(request.as_bytes(), peer(), &edge_state, &edge_config()).await;
    assert_eq!(datagrams.len(), 1);
    ...
    assert!(response.contains("Contact: <sip:1001@192.0.2.10:5070;transport=udp>;expires=120\r\n"));
...
async fn register_query_returns_existing_contact() {
    ...
    let datagrams = handle_datagram(query.as_bytes(), peer(), &edge_state, &edge_config()).await;
...
async fn invite_to_registered_contact_bypasses_gateway_routes() {
    ...
    assert!(
        outbound_invite.starts_with("INVITE sip:1002@192.0.2.20:5070;transport=udp SIP/2.0\r\n"),
        "{outbound_invite}"
    );
...
#[tokio::test]
async fn inbound_did_to_offline_extension_does_not_fall_back_to_public_route() {
    let edge_state = state_with_default_route();
    edge_state.replace_gateway_cache([("192.0.2.10".to_string(), "carrier-a".to_string())]);
    edge_state
        .call_manager
        .update_caller_numbers(call_core::CallerNumberDirectory::new([(
            "4008001".to_string(),
            "carrier-a".to_string(),
        )]));
    edge_state.replace_did_destinations(HashMap::from([(
        ...
        cdr_core::DidDestination {
            ...
            target_id: "1002".to_string(),
            enabled: true,
            updated_at: time::OffsetDateTime::now_utc(),
        },
    )]));
    let invite = concat!(
        "INVITE sip:4008001@example.com SIP/2.0\r\n",
        "Via: SIP/2.0/UDP 192.0.2.10:5060;branch=z9hG4bK-did-offline\r\n",
        "Max-Forwards: 70\r\n",
        "From: <sip:8613800138000@example.com>;tag=from-tag\r\n",
        "To: <sip:4008001@example.com>\r\n",
        "Call-ID: invite-did-offline@example.com\r\n",
        "CSeq: 1 INVITE\r\n",
        "Content-Length: 0\r\n",
        "\r\n"
    );
    ...
    assert_eq!(datagrams.len(), 1);
    let response = datagram_text(&datagrams[0]);
    assert!(response.starts_with("SIP/2.0 480 Temporarily Unavailable"));
    assert!(!response.contains("gw1.example.com"));
    assert!(
        edge_state
            .call_manager
            .get(&CallId::new("invite-did-offline@example.com"))
            .is_none(),
        "offline DID must not enter the public route table"
    );
}

#[tokio::test]
async fn test_invite_did_routing_to_extension_group() {
    let edge_state = state_with_default_route();
    edge_state.replace_gateway_cache([("192.0.2.10".to_string(), "carrier-a".to_string())]);
    edge_state
        .call_manager
    ...
    edge_state.replace_did_destinations(HashMap::from([(
        "4008003".to_string(),
        cdr_core::DidDestination {
            number: "4008003".to_string(),
            tenant_id: None,
            target_type: "extension_group".to_string(),
    ...
    let invite = concat!(
        ...
        "Max-Forwards: 70\r\n",
        "From: <sip:8613800138000@example.com>;tag=from-tag\r\n",
        "To: <sip:4008003@example.com>\r\n",
        "Call-ID: invite-did-group@example.com\r\n",
        "CSeq: 1 INVITE\r\n",
        "Content-Length: 0\r\n",
        "\r\n"
    );
...
async fn test_invite_did_routing_to_ivr() {
    ...
    edge_state.replace_did_destinations(HashMap::from([(
        "4008004".to_string(),
        cdr_core::DidDestination {
            number: "4008004".to_string(),
            tenant_id: None,
            target_type: "ivr".to_string(),
            target_id: "menu-1".to_string(),
            enabled: true,
            updated_at: time::OffsetDateTime::now_utc(),
        },
    )]));
    ...
    let invite = format!(
        "INVITE sip:4008004@example.com SIP/2.0\r\n\
         Via: SIP/2.0/UDP 192.0.2.10:5060;branch=z9hG4bK-did-ivr\r\n\
         Max-Forwards: 70\r\n\
         From: <sip:8613800138000@example.com>;tag=from-tag\r\n\
         To: <sip:4008004@example.com>\r\n\
         Call-ID: invite-did-ivr@example.com\r\n\
         CSeq: 1 INVITE\r\n\
...
async fn test_out_of_dialog_message_routing_to_registered_contact() {
    ...
    assert!(forwarded_msg.starts_with("MESSAGE sip:1001@192.0.2.10:5070;transport=udp SIP/2.0\r\n"));
...
async fn test_graceful_shutdown_draining_invite_receives_503() {
    ...
    let options = "OPTIONS sip:edge@example.com SIP/2.0\r\n\
                       ...
                       CSeq: 1 OPTIONS\r\n\
                       Content-Length: 0\r\n\r\n";
    let datagrams = handle_datagram(options.as_bytes(), peer(), &edge_state, &edge_config()).await;
    assert_eq!(datagrams.len(), 1);
    let resp = String::from_utf8_lossy(&datagrams[0].bytes);
...
async fn pairs_rtp_relay_ports_after_sdp_offer_answer() {
    ...
    let transaction = edge_state
        .inbound_transactions
        .get("invite-sdp-pair@example.com")
        .expect("transaction should be remembered");
    assert_eq!(
...
async fn in_dialog_request_with_wrong_from_tag_receives_481() {
    ...
    assert!(response.contains("X-VOS-RS-Error: in-dialog From tag does not match call dialog\r\n"));
}
...
async fn in_dialog_request_with_wrong_to_tag_receives_481() {
    ...
    let response = datagram_text(&datagrams[0]);
    assert!(response.starts_with("SIP/2.0 481 Call/Transaction Does Not Exist\r\n"));
    assert!(response.contains("X-VOS-RS-Error: in-dialog To tag does not match call dialog\r\n"));
}
...
async fn in_dialog_request_with_stale_cseq_receives_server_error_without_forwarding() {
    ...
    assert!(
        response.contains("X-VOS-RS-Error: out-of-order in-dialog CSeq: received 2, last 2\r\n")
    );
...
async fn inbound_cancel_gets_ok_and_is_forwarded_to_gateway() {
    ...
    let datagrams = handle_datagram(cancel.as_bytes(), peer(), &edge_state, &edge_config()).await;
...
async fn test_path_service_route_propagation() {
    ...
    assert!(resp.starts_with("SIP/2.0 200 OK\r\n"));
    let expected_service_route = format!("Service-Route: <sip:{};lr>\r\n", config.advertised_addr);
...
async fn test_parallel_ringing_call_forking() {
...

    {
        ...
        assert_eq!(
            health.health("gw1").map(|h| h.active_calls()).unwrap_or(0),
            1
        );
        assert_eq!(
            health.health("gw2").map(|h| h.active_calls()).unwrap_or(0),
            1
        );
    }
...
async fn test_tenant_domain_isolation() {
    ...
    let datagrams =
        handle_datagram(cross_invite.as_bytes(), peer(), &edge_state, &edge_config()).await;
...
"#;
    ...
    assert_eq!(
        config.database_url,
        Some("postgres://tangyu@127.0.0.1:5432/vos_rs".to_string())
    );
...
async fn rewrites_invite_sdp_offer_for_gateway_media_relay() {
    ...
    let transaction = edge_state
        .inbound_transactions
        .get("invite-sdp-offer@example.com")
        .expect("transaction should be remembered");
    assert_eq!(
...
async fn rewrites_gateway_answer_sdp_for_caller_media_relay() {
    ...
    let transaction = edge_state
        .inbound_transactions
        .get("invite-sdp-answer@example.com")
        .expect("transaction should be remembered");
    assert_eq!(
...
async fn test_media_hold_renegotiation() {
    ...
    let rewritten =
        media::rewrite_sdp_body(raw_sdp.as_bytes(), RtpEndpoint::new("127.0.0.1", 40000)).unwrap();
...
async fn test_nat_traversal_registered_contact_override() {
    ...
    assert!(
        forwarded_msg.starts_with("INVITE sip:1001@192.168.1.100:5060;transport=udp SIP/2.0\r\n")
    );
}
...
async fn test_sbc_rules_dynamic_reload() {
    ...
    assert!(edge_state
        .sbc_engine
        .is_allowed("192.0.2.50".parse().unwrap()));
...
async fn test_sbc_dynamic_lock_brute_force() {
    ...
    for i in 0..5 {
        ...
        let res = handle_datagram(
            packet_wrong_auth.as_bytes(),
            peer_addr,
            &edge_state,
            &config,
        )
...
async fn test_session_refresh_resets_on_reinvite() {
    ...
    let before_reinvite = {
        let tx_guard = edge_state.inbound_transactions.get(call_id).unwrap();
        tx_guard.last_session_refresh
    };
...
async fn test_early_media_183_sdp_allocates_relay_and_forwards() {
    ...
    assert_eq!(
        datagrams_200.len(),
        2,
        "gateway ACK and caller 200 are required"
    );
...
async fn inbound_refer_gets_accepted_and_notify_progress() {
    ...
    assert!(forwarded.starts_with("INVITE sip:1002@gw1.example.com:5060;transport=udp SIP/2.0\r\n"));
...
async fn gateway_refer_gets_accepted_notify_and_forwarded_to_caller() {
    ...
    let forwarded = datagram_text(&datagrams[2]);
    assert!(forwarded.starts_with("INVITE sip:1003@gw1.example.com:5060;transport=udp SIP/2.0\r\n"));
    assert!(forwarded.contains("CSeq: 1 INVITE\r\n"));
}
...
async fn test_refer_local_transfer_lifecycle() {
    ...
    assert!(invite_c.starts_with("INVITE sip:1002@gw1.example.com:5060;transport=udp SIP/2.0\r\n"));
...
async fn test_playback_and_mute_control_flow() {
    ...
    let (len, from) =
        tokio::time::timeout(Duration::from_millis(500), receiver.recv_from(&mut buffer))
            .await
            .expect("接收 RTP 播放包超时")
            .expect("读取数据失败");
...
async fn test_database_connection() {
    ...
    if let Some(ref db_url) = config.database_url {
        let connect_result =
            cdr_core::PostgresCdrStore::connect(db_url, config.database_max_connections).await;
        assert!(
...
async fn test_redis_connection() {
    ...
    if let Some(ref redis_url) = config.redis_url {
        ...
        assert!(
            conn.is_ok(),
            "Failed to establish Redis multiplexed connection: {:?}",
            conn.err()
        );
    } else {
...
async fn test_s3_storage_connection() {
    ...
    assert!(
        create_result.is_ok(),
        "S3/Storage backend creation test failed: {:?}",
        create_result.err()
    );
}
...
async fn test_sip_invite_join_and_leave_conference() {
    ...
    // 验证参会成员已被加入会议
    let conf_exists = edge_state
        .media_relay
        .conference_manager
        .conferences
        .contains_key("conf_room1");
    assert!(conf_exists);

    // 获得分配的媒体端口
    let mut local_port = 0;
    if let Some(entry) = edge_state
        .media_relay
        .conference_manager
        .conferences
        .get("conf_room1")
    ...
    let conf_exists_after = edge_state
        .media_relay
        .conference_manager
        .conferences
        .contains_key("conf_room1");
...
async fn test_conference_participant_muting() {
    ...
    // Mute participant 1
    let success = conf_manager
        .set_participant_mute(conference_id, port1, true)
        .await;
    assert!(success);
...
async fn test_call_monitoring() {
    ...
    client_sender
        .send_to(&rtp_payload, format!("127.0.0.1:{}", port))
        .unwrap();
    ...
    let Ok(Ok((received_size, _from_addr))) =
        tokio::time::timeout(Duration::from_millis(1000), read_future).await
    else {
...
async fn test_http_webhook_call_control() {
    ...
    use call_core::{CallEvent, VciInstruction, WebhookEvent};
    use std::sync::Arc;
    ...
    let app = Router::new().route(
        "/webhook",
        post(move |Json(event): Json<WebhookEvent>| {
    ...
    let (port_min, port_max) = get_thread_ports();
    edge_config.media.port_min = port_min;
    edge_config.media.port_max = port_max;
...
async fn test_http_webhook_stream_instruction() {
    use axum::{routing::post, Json, Router};
    use call_core::{VciInstruction, WebhookEvent};
    use std::sync::Arc;
    ...
    let app = Router::new().route(
        "/webhook",
        post(move |Json(_event): Json<WebhookEvent>| {
...
async fn test_sip_flow_packet_capture() {
    ...
    assert!(
        result.is_err(),
        "Non-whitelisted call should not be captured"
    );
}
...
async fn test_nats_webhook_call_control() {
    ...
    let mut subscriber = nats_client
        .subscribe("test.vos_rs.call.incoming".to_string())
        .await
        .unwrap();
        ...
        if let Some(msg) = subscriber.next().await {
            ...
            if let Some(reply_to) = msg.reply {
                let _ = nats_client_clone
                    .publish(reply_to, reply_payload.into())
                    .await;
            }
        }
    });
