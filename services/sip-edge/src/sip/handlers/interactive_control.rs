...
use crate::edge_state::{EdgeState, PendingDatagram, ParkedCall};
use crate::config::EdgeConfig;
use crate::sip::handlers::response_for_media_error;
use call_core::{WebhookEvent, CallEvent, VciInstruction, WEBHOOK_SCHEMA_VERSION};
use std::str::FromStr;
...
pub(crate) async fn handle_interactive_webhook_call(
    ...
    if let Some(instruction) = post_webhook_event(edge_state, edge_config, &event).await {
        execute_instruction(instruction, call_id.clone(), Arc::clone(edge_state), edge_config.clone()).await;
    } else {
        ...
        crate::sip::handlers::command_listener::handle_command(hangup_cmd, edge_state, edge_config).await;
    }
...
async fn answer_parked_call_if_needed(
    ...
    edge_state.remember_inbound_invite(
        ...
        parked.peer_addr,
        sip_core::SipUri::from_str(&format!("sip:{}@{}", service_name, edge_config.advertised_addr)).unwrap(),
        client_ep.clone(),
        Some(local_ep.clone()),
        false,
        None,
    ...
    let resp = crate::sip::response::build_response_with_owned_headers(
        ...
        &[
            ...
            ("Contact".to_string(), format!("<sip:{}@{}>", service_name, edge_config.advertised_addr)),
        ],
        &sdp_answer,
    );
    ...
    edge_state.media_relay.register_port_codec(parked.caller_relay_port, codec);
    if let Some(ref client_rtp) = client_ep {
        let _ = edge_state.media_relay.set_target(
            &sdp_core::RtpEndpoint { address: edge_config.media.advertised_addr.clone(), port: parked.caller_relay_port },
            client_rtp
        );
    }
...
pub(crate) async fn post_webhook_event(
    ...
    if edge_config.webhooks.control_mode == "nats" {
        ...
        match response {
            Ok(Ok(reply)) => {
                match serde_json::from_slice::<VciInstruction>(&reply.payload) {
                    Ok(instruction) => Some(instruction),
                    Err(e) => {
                        warn!("failed to deserialize VciInstruction from NATS reply: {:?}", e);
                        None
                    }
                }
            }
        ...
        match response {
            Ok(resp) if resp.status().is_success() => {
                match resp.json::<VciInstruction>().await {
                    Ok(inst) => Some(inst),
                    Err(e) => {
                        warn!("failed to deserialize VciInstruction from Webhook response: {:?}", e);
                        None
                    }
                }
            }
...
pub(crate) fn execute_instruction(
    ...
    if !is_parked && !is_active {
        warn!(call_id, "call not found in parked or active calls, ignoring instruction");
        return;
    }

    match instruction {
        VciInstruction::Dial { targets, sim_ring: _, caller_id, timeout_secs, record_call: _ } => {
        ...
        VciInstruction::Play { url, loop_count } => {
            let port = if edge_state.parked_calls.contains_key(call_id) {
                answer_parked_call_if_needed(call_id, edge_state, edge_config, "playback", "vosrs-playback-tag").await
            } else {
                edge_state.inbound_transactions.get(call_id).and_then(|tx| tx.caller_relay_rtp.as_ref().map(|ep| ep.port))
            };
                ...
                tokio::spawn(async move {
                    ...
                    loop {
                        ...
                        if !edge_state_clone.media_relay.playbacks.contains_key(&port) {
                            ...
                            if let Some(next_inst) = post_webhook_event(&edge_state_clone, &edge_config_clone, &event).await {
                                execute_instruction(next_inst, cid.clone(), Arc::clone(&edge_state_clone), edge_config_clone.clone()).await;
                            }
                            break;
                        }
                    }
        ...
        VciInstruction::Gather { play_url, max_digits, timeout_ms, inter_digit_timeout_ms: _, finish_on_key: _, barge_in: _ } => {
            let port = if edge_state.parked_calls.contains_key(call_id) {
                answer_parked_call_if_needed(call_id, edge_state, edge_config, "gather", "vosrs-gather-tag").await
            } else {
                edge_state.inbound_transactions.get(call_id).and_then(|tx| tx.caller_relay_rtp.as_ref().map(|ep| ep.port))
            };

            if let Some(port) = port {
                edge_state.media_relay.register_port_dtmf_tracking(call_id, port, 101);
                ...
                tokio::spawn(async move {
                    ...
                    let event = WebhookEvent {
                        ...
                        occurred_at_ms: SystemTime::now()
                            ...
                            .unwrap_or_default()
                            .as_millis() as i64,
                        event: CallEvent::DtmfReceived { digits: gathered, leg: "a_leg".to_string() },
                    };
                    if let Some(next_inst) = post_webhook_event(&edge_state_clone, &edge_config_clone, &event).await {
                        execute_instruction(next_inst, cid.clone(), Arc::clone(&edge_state_clone), edge_config_clone.clone()).await;
                    }
        ...
        VciInstruction::Hangup { reason_code: _, sip_cause } => {
            let cmd = crate::sip::handlers::command_listener::CallCommand {
                call_id: call_id.to_string(),
                action: crate::sip::handlers::command_listener::CommandAction::Hangup {
                    params: crate::sip::handlers::command_listener::HangupParams {
        ...
        VciInstruction::Stream { websocket_url, format, barge_in } => {
            let cmd = crate::sip::handlers::command_listener::CallCommand {
                call_id: call_id.to_string(),
        ...
        VciInstruction::Record { max_length_secs, play_beep, trim_silence: _, silence_threshold_db: _ } => {
            ...
            if let Some((Some(caller_port), Some(gateway_port))) = tx_opt {
                ...
                tokio::spawn(async move {
                    ...
                    edge_state_clone.media_relay.recordings.remove(&gateway_port);
                    info!(call_id = %cid, "call recording stopped after reaching max_length_secs");
                });
            } else {
                warn!(call_id, "call must be active and bridged to start recording");
            }
        }

        VciInstruction::Say { text, voice, speed: _, pitch: _ } => {
        ...
        VciInstruction::Queue { queue_id: _, moh_url, priority: _ } => {
            info!(call_id, moh_url, "VCI Queue command received, putting call in queue loop");
            let cmd = VciInstruction::Play {
                url: moh_url,
                loop_count: 9999,
            };
            execute_instruction(cmd, call_id.clone(), Arc::clone(edge_state), edge_config.clone()).await;
        }

        VciInstruction::Conference { room_id, start_muted: _, end_on_exit: _, max_participants: _ } => {
            let port = if edge_state.parked_calls.contains_key(call_id) {
                answer_parked_call_if_needed(call_id, edge_state, edge_config, "conference", "vosrs-conference-tag").await
            } else {
                edge_state.inbound_transactions.get(call_id).and_then(|tx| tx.caller_relay_rtp.as_ref().map(|ep| ep.port))
            };
            if let Some(port) = port {
                let target_addr_opt = if let Some(tx) = edge_state.inbound_transactions.get(call_id) {
                    if let Some(ref caller_rtp) = tx.caller_rtp {
                        format!("{}:{}", caller_rtp.address, caller_rtp.port).parse::<SocketAddr>().ok()
                    } else {
                        None
                    }
                } else {
                ...
                if let Some(target_addr) = target_addr_opt {
                    let codec = edge_state.media_relay.codecs.get(&port).map(|c| *c.value()).unwrap_or(rtp_core::AudioCodec::Pcma);
                    if let Some(socket) = edge_state.media_relay.active_sockets.get(&port).map(|s| s.value().clone()) {
                        let _ = edge_state.media_relay.conference_manager.join_conference(
                            &room_id,
                            port,
                            codec,
                            target_addr,
                    ...
                    warn!(call_id, "failed to get caller target address for conference");
                }
            }
        }
        ...
        VciInstruction::Redirect { url } => {
            ...
            tokio::spawn(async move {
                ...
                if let Some(next_inst) = post_webhook_event(&edge_state_clone, &edge_config_clone, &event).await {
                    execute_instruction(next_inst, cid.clone(), Arc::clone(&edge_state_clone), edge_config_clone).await;
                }
            });
        }
            ...
            tokio::spawn(async move {
                ...
                let event = WebhookEvent {
                    ...
                    occurred_at_ms: SystemTime::now()
                        ...
                        .unwrap_or_default()
                        .as_millis() as i64,
                    event: CallEvent::CallAnswered { sip_status: 200, leg: "a_leg".to_string() },
                };
                if let Some(next_inst) = post_webhook_event(&edge_state_clone, &edge_config_clone, &event).await {
                    execute_instruction(next_inst, cid.clone(), Arc::clone(&edge_state_clone), edge_config_clone.clone()).await;
                }
        ...
        VciInstruction::PlayDigits { digits, duration_ms: _ } => {
            info!(call_id, digits, "VCI PlayDigits command received, sending SIP INFO");
                ...
                if let Some(socket) = edge_state.get_socket() {
                    if let Some(ref orig_req) = tx.original_request {
                        ...
                        for digit in digits.chars() {
                            ...
                            let info_bytes = crate::sip::outbound::build_outbound_in_dialog_request(
                                &dummy_req,
                                &tx.outbound_uri,
                            ...
                            let target_peer = tx.outbound_peer.clone().unwrap_or_else(|| tx.peer.clone());
