...
分机呼叫、DID 呼入、落地选择、计费和 CDR 的目标设计。已接通能力和剩余差距
以第 13 节为准。
...
截至 2026-07-18，接入/落地中继角色、IP/CIDR 认证规则、第三方 Digest 注册、
号码唯一落地归属、接入中继或分机独占授权、严格透传/固定号码/虚拟池策略、
落地直绑/分组、DID 到分机或拒绝、脉冲计费、CDR 决策审计、号码与中继 Redis
原子容量租约以及录音播放已经接入 API、数据库、运行时或管理页面。Realm 由系统
配置统一提供，修改用户名时必须重新输入密码；号码池成员保存时会校验来源授权、
显号方向、状态和落地归属。

仍未完成的能力如下，管理页面不会把它们伪装成可用选项：

- 平台主动向上游发起 REGISTER 的落地中继客户端状态机。
- 分机组、IVR 等 DID 目标的完整运行时。
- 跨节点共享轮询游标、超长通话租约续期，以及异步释放失败后的主动补偿。
- 满载号码池成员的自动二次选号，以及旧模型跨中继故障切换时的原子容量迁移。
- 历史独立路由模型的最终下线；当前保留兼容入口，新的日常配置使用落地绑定。
