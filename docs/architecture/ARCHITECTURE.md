...
## 三、各子系统核心逻辑与 VOS 逐项对比

### 3.1 信令解析与事件调度 (sip-core / sip-edge)
...
| 实时音频转码 | **LiveTranscoder 协程局部转码引擎**（利用 `opus` 静态链接库与 `rubato` 重采样，结合 `fifo` 环形缓冲在物理转发循环原地双向转码，无全局锁） | VOS 外置转码卡或转码节点 |
| G.711 加速 | **LUT 查表法与 Inplace 原地转码**（引入 256 字节 `OnceLock` 缓存表，G.711 转换变为 $O(1)$ L1-Cache 原地改写，零分配） | VOS 内置查表或硬件加速 |
| 放音异步化 | **WAV 磁盘 I/O 异步化**（通过 `spawn_blocking` 将文件读取彻底从 Tokio 网络核心 Worker 中隔离） | VOS 同 |
| 录音 | 双声道 WAV 8kHz/16bit，支持异步 Worker 线程池写入 | VOS 支持多格式 + 云存储 |
...
**CDR 数据流**：
```
sip-edge → CallManager.completed_cdrs → flush_completed_cdrs → PostgreSQL (基于 UNNEST 静态批量数组写入)
         → 或 → NatsCdrPublisher → NATS JetStream → cdr-worker → PostgreSQL
```

**已优化与设计要点**：
- **UNNEST 批量写入**：用 SQL 的 `UNNEST` 重构了 CDR 数据批量插入。相比 QueryBuilder 的动态拼接，在多话单写入时避免了内存频繁申请和语法解析耗时，性能大提升。
...
| **实时音频转码** | 外部转码卡或集群 | ✅ 已完整支持 (引入基于 `opus` 和 `rubato` 的协程本地 LiveTranscoder，双向对齐重采样) | 已完成 |
| **WAV 磁盘 I/O 隔离** | 流媒体隔离载入 | ✅ 已完整支持 (使用 `spawn_blocking` 将 WAV 磁盘读取从网络核心协程中分离) | 已完成 |
| **零拷贝解析** | 高速 C 语言解析 | ✅ 已完整支持 (SIP 头部、URI 及报文解析完全零拷贝借用缓冲区生命周期) | 已完成 |
...
| 问题 | 说明 | 修复与改进措施 |
|------|------|----------------|
| **SIP 信令解析拷贝** | 信令转换伴随大量堆内存 String/Vec 拷贝 | **已优化**：引入零拷贝生命周期借用类型，消除报文解析时的堆分配开销 |
| **CDR 批量写 Postgres 锁** | 动态 QueryBuilder 拼接高频 SQL 导致解析器负载极高 | **已优化**：重构为 PostgreSQL 静态 `UNNEST` 批量数组绑定写入模式，写入效率大幅度提升 |
| **WAV 放音同步 I/O** | 播音时读取 wav 文件造成 Tokio 网络协程挂起 | **已优化**：将 WAV 文件 I/O 转移至 `spawn_blocking` 后台线程池进行异步载入 |
| **音频转码锁开销** | 实时转码通道在全局锁中频繁检索 | **已优化**：将 `LiveTranscoder` 绑定为协程局部变量，实现高并发无锁转码 |
| **端口分配 Mutex 锁** | 原顺序分配占用全局锁，bind 阻塞严重 | **已优化**：引入基于有界原子位图 (PortLeaseMap) 的无锁 (Lock-free) 偶奇优先端口分配机制 |
| **呼叫保持与协商** | 原重协商时流匹配混乱 | **已优化**：增加了显式的 Dialog 会话状态重谈判与 SSRC 追踪 |
| **呼叫转移 (REFER)** | 原转接易出现状态失步 | **已优化**：引入完整的 REFER 状态机与松散路由 (Loose Routing) 支撑 |
| **路由无权重** | 原仅按 Cost 最低成本路由排序 | **已优化**：已支持 weight 字段与加权随机负载均衡选路 |
| **录音文件名易冲突** | 原使用时间戳命名 | **已优化**：改用 UUID `call_id.wav` 结合分段轮转机制 |
...
系统已删除所有分散的环境变量，统一通过唯一环境变量 `VOS_RS_CONFIG_FILE` 指定引导配置文件（默认路径为 `config.yaml`），在其中配置连接池、数据库与 Redis 端口、S3 对象存储以及 sip-edge 网络 and 路由选项。
...
2. **零拷贝 SIP 协议解析**：引入借用生命周期类型，彻底消除了信令报文堆内存拷贝。
3. **CDR UNNEST 批量写入**：摒弃动态 SQL，改用 PostgreSQL UNNEST 数组绑定进行高速批量写入。
4. **端口分配优化**：已引入基于原子位图 (PortLeaseMap) 的 Lock-free 偶奇优先端口分配机制，规避 bind 系统调用阻塞 Mutex。
5. **注册集群同步**：已将注册绑定同步写入 Redis 并设置 TTL，多节点运行可以相互路由。

### P1 — 功能补全与媒体增强（已完成）
1. **实时音频转码**：引入 `opus` 与 `rubato` 实现了 Opus(48k) ↔ G.711(8k) 双向实时转码。
2. **放音异步化与 LUT 优化**：将 WAV 音频加载移至 `spawn_blocking` 隔离运行；引入 LUT 查表实现 G.711 常数级原地改写。
3. **呼叫转移**：盲转（REFER）与 attended transfer 已完整支持。
4. **呼叫保持**：Re-INVITE 媒体重新协商与 RTP 挂起已完成。
5. **实时计费与拆线**：已支持呼叫前余额校验、最大允许时长监测以及超时强制 BYE 报文硬拆线。
6. **录音格式转码**：已实现挂断后通过 ffmpeg 异步将 WAV 转码为 Opus/AMR 压缩格式并清理原始 WAV 文件。
7. **会议桥**：已实现拨号直接加入会议室、UAS 协商、20ms 内 RTP 解码混音（Mix-Minus）与挂断时自动 leaves 资源清理。
