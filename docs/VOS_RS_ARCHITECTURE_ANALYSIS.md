...
| 4 | **SBC 限速用 Mutex** | 无锁数据结构 | Mutex 在高并发下成为瓶颈，改用 DashMap |
| 5 | **用户并发数遍历 DashMap** | 维护计数器 | 每次 INVITE 遍历所有事务效率低，应维护 per-user 计数器 |
| 6 | **认证 secret_key 启动时随机生成** | 配置固定密钥 | 多实例部署时密钥不一致，应从配置读取 |
...
| # | 优化点 | 预期收益 | 实现难度 |
|---|--------|----------|----------|
| 1 | **UDP 发送缓冲区** | 已优化 (4MB) | 低 |
| 2 | **DashMap 分片** | 已使用 | 低 |
| 3 | **录音异步写入** | 减少 I/O 阻塞 | 中 |
| 4 | **CDR 批量写入** | 减少数据库往返 | 中 |
| 5 | **路由表缓存** | 减少数据库查询 | 低 |
| 6 | **SBC 限速无锁化** | 提升高并发性能 | 中 |
| 7 | **Rust 编译优化** | LTO + codegen-units=1 | 低 |
| 8 | **连接池** | PostgreSQL 连接复用 | 低 |
