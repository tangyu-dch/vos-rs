...
- 路由选择：`call-core/routing.rs` → 最长前缀匹配 → LCR（最低成本路由）
...
**路由选择流程**：
1. 从 `sip_routes` 表加载路由规则
2. 按被叫号码最长前缀匹配
3. 同前缀按优先级排序（数字越小越优先）
4. 同优先级按 cost 升序（LCR 最低成本路由）
5. 检查时间窗口（time_start/time_end）
6. 检查网关健康状态（GatewayHealthTracker）
7. 检查并发容量（max_capacity / current_concurrent）

**前缀规则**：`abc:def`（替换）、`:def`（添加前缀）、`abc:`（剥离前缀），逗号分隔多条。

**VOS 对比**：
- VOS 有完整的 LCR + 时间路由 + 负载均衡 + 故障转移
- VOS-RS 缺少：权重负载均衡、故障转移优先级、基于 ACD/ASR 的动态路由
...
| **路由无权重** | LCR 只按 cost 排序，无权重轮询 | 增加 weight 字段 |
...
| `sip_routes` | 路由规则 | prefix, priority, gateway_id, cost, time_start/end |
