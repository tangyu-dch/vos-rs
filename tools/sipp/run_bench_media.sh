...
run_one() {
  ...
  # Start sip-edge
  env -u VOS_RS_DATABASE_URL -u VOS_RS_NATS_URL \
  VOS_RS_SIP_UDP_BIND="127.0.0.1:$EP" \
  VOS_RS_SIP_DEFAULT_GATEWAY="127.0.0.1:$GP" \
  VOS_RS_SIP_ADVERTISED_ADDR="127.0.0.1:$EP" \
  VOS_RS_RTP_PORT_MIN=$RTP_MIN VOS_RS_RTP_PORT_MAX=$RTP_MAX \
  VOS_RS_SIP_UDP_RECEIVE_BUFFER="$SIP_UDP_RECEIVE_BUFFER" \
  VOS_RS_SIP_UDP_SEND_BUFFER="$SIP_UDP_SEND_BUFFER" \
  VOS_RS_SBC_ALLOW=127.0.0.1 VOS_RS_SBC_LIMIT_CAPACITY=1000000 \
  VOS_RS_SBC_LIMIT_FILL_RATE=100000 VOS_RS_SBC_MAX_CONCURRENCY=20000 \
  VOS_RS_RECORDING_ENABLED="$MEDIA_RECORDING_ENABLED" \
  VOS_RS_RECORDING_DIR="$MEDIA_RECORDING_DIR" \
  VOS_RS_RECORDING_WORKERS="$RECORDING_WORKERS" \
  VOS_RS_RECORDING_QUEUE_CAPACITY="$RECORDING_QUEUE_CAPACITY" \
  VOS_RS_MEDIA_METRICS_LOG=true \
