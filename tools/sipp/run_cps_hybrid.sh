...
for pause_ms in $DURATIONS; do
  for direction in outbound inbound; do
    scenario_file="$TMP_SCENARIOS/cps_${direction}_${pause_ms}.xml"
    from_header='"1001" <sip:1001@[local_ip]:[local_port]>;tag=[pid]vosrsCps[call_number]'
    if [ "$direction" = "inbound" ]; then
      from_header='"ext-2001" <sip:2001@[local_ip]:[local_port]>;tag=[pid]vosrsCps[call_number]'
    fi

    cat > "$scenario_file" << XMLEOF
<?xml version="1.0" encoding="ISO-8859-1" ?>
<!DOCTYPE scenario SYSTEM "sipp.dtd">
<scenario name="CPS ${direction} ${pause_ms}ms">
  <send retrans="500">
    <![CDATA[
...
# Start RTP sender (监听 SIP 200 OK 提取 relay 地址，发送 RTP)
...
DURATIONS_ARRAY=($DURATIONS)
NUM_DURATIONS=${#DURATIONS_ARRAY[@]}
OUTBOUND_PER_DUR=$((OUTBOUND_CALLS / NUM_DURATIONS))
INBOUND_PER_DUR=$((INBOUND_CALLS / NUM_DURATIONS))
OUTBOUND_REMAINDER=$((OUTBOUND_CALLS - OUTBOUND_PER_DUR * NUM_DURATIONS))
INBOUND_REMAINDER=$((INBOUND_CALLS - INBOUND_PER_DUR * NUM_DURATIONS))
...
PIDS=()
for pause_ms in $DURATIONS; do
  calls=$((OUTBOUND_PER_DUR))
  [ $OUTBOUND_REMAINDER -gt 0 ] && calls=$((calls + 1)) && OUTBOUND_REMAINDER=$((OUTBOUND_REMAINDER - 1))
  if [ $calls -gt 0 ]; then
    "$SIPP_BIN" "$LOCAL_IP:$EDGE_PORT" -sf "$TMP_SCENARIOS/cps_outbound_${pause_ms}.xml" \
      -i "$LOCAL_IP" -m "$calls" -r "$CALL_RATE" -l 50 -nostdin \
      -trace_err -error_file "$LOG_DIR/err_out_${pause_ms}.log" \
      >"$LOG_DIR/sipp_out_${pause_ms}.stdout" 2>&1 &
    PIDS+=($!)
  fi

  calls=$((INBOUND_PER_DUR))
  [ $INBOUND_REMAINDER -gt 0 ] && calls=$((calls + 1)) && INBOUND_REMAINDER=$((INBOUND_REMAINDER - 1))
  if [ $calls -gt 0 ]; then
    "$SIPP_BIN" "$LOCAL_IP:$EDGE_PORT" -sf "$TMP_SCENARIOS/cps_inbound_${pause_ms}.xml" \
      -i "$LOCAL_IP" -m "$calls" -r "$CALL_RATE" -l 50 -nostdin \
      -trace_err -error_file "$LOG_DIR/err_in_${pause_ms}.log" \
      >"$LOG_DIR/sipp_in_${pause_ms}.stdout" 2>&1 &
    PIDS+=($!)
  fi
done

echo "  Launched ${#PIDS[@]} SIPp instances + RTP sender"
